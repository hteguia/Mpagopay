...
namespace Mpagopay.App.Auth
{
    public class CustomAuthenticationStateProvider : AuthenticationStateProvider
    {
        private readonly ILocalStorageService _loggerStorage;
        public CustomAuthenticationStateProvider(ILocalStorageService loggerStorage)
        {
        ...
        {
            var savedToken = await _loggerStorage.GetItemAsync<string>("token");
            ...
            return new AuthenticationState(new System.Security.Claims.ClaimsPrincipal(new ClaimsIdentity(ParseTokenClaims(savedToken), "jwt")));
        }
        ...
        private IEnumerable<Claim> ParseTokenClaims(string jwt)
        {
            ...
            var jsonBytes = ParseBase64WithoutPading(playload);
            ...
            if(roles != null)
            {
                if (roles.ToString().Trim().StartsWith("["))
                {
                    ...
                    foreach(var parsedRol in parsedRoles)
                    {
                        claims.Add(new Claim(ClaimTypes.Role, parsedRol));
                    }
                }
                else
                {
